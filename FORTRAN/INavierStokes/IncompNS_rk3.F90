...
#include "Solver.h"
       ...
       call Diffusive_U(u,gr_dx_nodes,gr_dy_centers,D1)

       G1 = C1 + D1

       if (tstep == 0) then

              ut(2:Nxb+1,2:Nyb+1)=u(2:Nxb+1,2:Nyb+1)+(gr_dt/1)*(G1)
              G1_old = G1
       else

              ut(2:Nxb+1,2:Nyb+1)=u(2:Nxb+1,2:Nyb+1)+(gr_dt/2)*(3*G1_old-G1)
              G1_old = G1
       endif


       call Convective_V(u,v,gr_dx_nodes,gr_dy_centers,C2)
       call Diffusive_V(v,gr_dx_centers,gr_dy_nodes,D2)

       G2 = C2 + D2

       if (tstep == 0) then

              vt(2:Nxb+1,2:Nyb+1)=v(2:Nxb+1,2:Nyb+1)+(gr_dt/1)*(G2)
              G2_old = G2
       else

              vt(2:Nxb+1,2:Nyb+1)=v(2:Nxb+1,2:Nyb+1)+(gr_dt/2)*(3*G2_old-G2)
              G2_old = G2
       endif
       ...
       p_RHS = -((1/(gr_dy_nodes(2:Nxb+1,2:Nyb+1)*gr_dt))*(vt(2:Nxb+1,2:Nyb+1)-vt(2:Nxb+1,1:Nyb)))&
               -((1/(gr_dx_nodes(2:Nxb+1,2:Nyb+1)*gr_dt))*(ut(2:Nxb+1,2:Nyb+1)-ut(1:Nxb,2:Nyb+1)))

       call Poisson_solver(p_RHS,p,ins_p_res,p_counter)

       u(2:Nxb+1,2:Nyb+1) = ut(2:Nxb+1,2:Nyb+1) - (gr_dt/gr_dx_centers(2:Nxb+1,2:Nyb+1))*(p(3:Nxb+2,2:Nyb+1)-p(2:Nxb+1,2:Nyb+1))
       v(2:Nxb+1,2:Nyb+1) = vt(2:Nxb+1,2:Nyb+1) - (gr_dt/gr_dy_centers(2:Nxb+1,2:Nyb+1))*(p(2:Nxb+1,3:Nyb+2)-p(2:Nxb+1,2:Nyb+1))
...
!! DIFFUSIVE U !!
subroutine Diffusive_U(ut,dx_nodes,dy_centers,D1)
...
!! DIFFUSIVE V !!
subroutine Diffusive_V(vt,dx_centers,dy_nodes,D2)
